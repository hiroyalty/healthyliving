...
var app = {
  ...
  rooms: function(){
    ...
    socket.on('connect', function () {
      ...
      $("#rm-clck").on('click', function(e) {
        ...
        var roomType = inputTy.val().trim();
        if(roomTitle !== '') {
          //socket.emit('createRoom', roomTitle);
          socket.emit('createRoom', roomTitle, roomType);
          inputEle.val('');
          inputTy.val('');
        }
      });
 ...
 // we can do initpublic room here also... if we dont want to make changes directly in the socket index page
 createpublic: function(){
     var socket = io('/createpublic', { transports: ['websocket'] });
    ...
    socket.on('connect', function () {
      ...
      $('.room-create button').on('click', function(e) {
        ...
        if(roomTitle !== '') {
          socket.emit('createRoom', roomTitle, roomType);
          //socket.emit('createPublicRoom', roomTitle, roomType);
          inputEle.val('');
          inputTy.val('');
        }
      });
