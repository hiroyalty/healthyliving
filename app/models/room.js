...
var create = function (data1, data2, callback){
	var newRoom = new roomModel(data1, data2);
	newRoom.save(callback);
}

var createpublic = function(data1, data2, callback){
    var newRoom = new roomModel(data1, data2);
    newRoom.save(callback);
}
...
var findOneAndUpdate = function(query, data, options, callback){
	//roomModel.findOneAndUpdate(id, data, { new: true });
        roomModel.findOneAndUpdate(query, data, options, callback);
}
...
var getUsers = function(room, socket, callback){
	...
	room.connections.forEach(function(conn){
                ...
                //number of sockets/connections d present user is having in one room
...
var removeUser = function(socket, callback){
	...
	find(function(err, rooms){
		...
		rooms.every(function(room){
			...
			// For every room, i: is the room id
			...
			if(!pass) {
				...
				room.save(function(err){
					callback(err, room, userId, cunt);
                                ...
                                //we shd also update the room, making it end....
			}
